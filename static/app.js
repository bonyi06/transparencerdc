...
// L'année/exercice doit toujours être visible par défaut : sans elle une
// ligne de recette est illisible ("perçu quand ?"). Avant ce correctif, une
// colonne de rôle 'year' tombait dans tier2 et n'était choisie que si les
// colonnes dimension/montant ne remplissaient pas déjà les `max` places —
// ce qui, pour les tables larges (ex. regie_dgi, 14 colonnes), l'excluait
// systématiquement de la vue par défaut (retour utilisateur, 10 sept. 2026).
// La colonne année est donc désormais toujours réservée en tête de liste.
function pickDefaultCols(name,max){
  max=max||7;
  const d=DS[name];if(!d)return [];
  const tierYear=[],tierFinal=[],tier1=[],tier2=[],tierIntermediate=[],tier3=[];
  d.cols.forEach(c=>{const r=columnRole(name,c);
    if(r==='year')tierYear.push(c);
    else if((r==='dimension'||r==='additive')&&FINAL_COL_RE.test(c))tierFinal.push(c);
  ...
  let chosen=tierYear.concat(tierFinal).concat(tier1).slice(0,max);
